...
from maddrive_adas.utils.general import LOGGER, colorstr
...
def select_device(device="", batch_size=0, newline=True):
    ...
    if cpu:
        ...
        assert nd > int(
            max(device.split(","))
        ), f"Invalid `--device {device}` request, valid devices are 0 - {nd - 1}"
        os.environ[
            "CUDA_VISIBLE_DEVICES"
        ] = device  # set environment variable (must be after asserts)
